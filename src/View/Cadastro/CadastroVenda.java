...
public class CadastroVenda extends javax.swing.JFrame {
    ...
    private void btnSelProdActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSelProdActionPerformed
        while(idClient == -1){
            ...
            op = openCliente();
            idClient = controlVenda.selecionarId(selCli.tabClientes);
            JOptionPane.showMessageDialog(selCli,"Selecione um cliente", "Selecione um cliente",ERROR);
        }if(idClient != -1){
            op = openProduto();
        }
    ...
    private void btnAddCarrinhoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddCarrinhoActionPerformed
        ...
        if(selProd.isClosed()){
            ...
            try {
                controlVenda.selecionarProduto(selProd.tabProds, x);
    ...
    private void btnRegistrarVendaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRegistrarVendaActionPerformed
        if(controlVenda.novaVenda(idClient, controlVenda.getItensVenda())){
            if(controlVenda.gravarVendaArquivo(controlVenda.getVenda() , true)){
    ...
    private void btnExcluirCarrinhoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirCarrinhoActionPerformed
        int x = selProd.tabCarrinho.getSelectedRow();
        int y = (Integer)selProd.tabCarrinho.getValueAt(x,0);
        //int id = controlVenda.selecionarId(selProd.tabCarrinho);
        // int id = controlVenda.selecionarId(selProd.tabCarrinho);
        System.out.println("id "+y);
        if(controlVenda.removerProdCarrinho(selProd.tabCarrinho, y)) {
            selProd.tabCarrinho = controlVenda.atualizaTabela( selProd.tabCarrinho , colunas);
            selProd.lblRes.setText(String.valueOf(controlVenda.valorTotal(selProd.tabCarrinho)   ));
        }else{
